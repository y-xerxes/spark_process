...
public class KafkaManager {
    ...
    public static Map<TopicAndPartition, Long> getOffsets(String zkKafkaServers,
        ...
        // in case offsets are blank for any partition
        if ("smallest".equalsIgnoreCase(kafkaParams.getOrDefault("auto.offset.reset", "smallest"))) {
            fillInEarliestOffsets(offsets, kafkaParams);
        } else {
    ...
    private static void fillInEarliestOffsets(Map<TopicAndPartition, Long> offsets, Map<String, String> kafkaParams) {
        if (offsets.containsValue(null)) {
            ...
            Map<TopicAndPartition, ?> leaderOffsets =
                    JavaConversions.mapAsJavaMap(kc.getEarliestLeaderOffsets(needOffsetScalaSet).right().get());
